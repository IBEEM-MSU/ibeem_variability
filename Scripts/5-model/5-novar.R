...
fit <- rstan::stan(paste0(dir, 'Scripts/Model_files/6-novar.stan'),
                   data = DATA,
                   chains = CHAINS,
                   iter = ITER,
                   cores = CHAINS,
...
MCMCvis::MCMCdiag(fit, 
                  ...
                  cp_file = c(paste0(dir, 'Scripts/Model_files/6-novar.stan'), 
                              paste0(dir, 'Scripts/6-novar.R')),
                  cp_file_names = c(paste0('6-novar-', run_date, '.stan'),
                                    paste0('6-novar-', run_date, '.R')))
...
pdf(paste0(fig_dir, 'param-cat-rs-', run_date, '.pdf'),
    height = 14, width = 5)
MCMCvis::MCMCplot(cbind(beta_rs_ch, 
                        gamma1_rs_ch,
                        gamma2_rs_ch,
                        gamma3_rs_ch,
                        theta1_rs_ch,
...
var_resid <- apply(DATA$y - mu_ch, 1, var)
...
var_resid_nm <- apply(DATA$y - mu_nm_ch, 1, var)
