####################
# Fit Bayes model - survival ~ env + phylo
...
MCMCvis::MCMCdiag(fit, 
                  ...
                  pg0 = TRUE,
                  save_obj = TRUE,
                  obj_name = paste0('se-bird-novar-surv-phylo-fit-', run_date),
                  add_obj = list(DATA),
                  add_obj_names = paste0('se-bird-novar-surv-phylo--data-', run_date),
                  cp_file = c(paste0(dir, 'Scripts/Model_files/5-novar-phylo.stan'), 
                              paste0(dir, 'Scripts/5-model/5-novar-surv-phylo.R')),
                  cp_file_names = c(paste0('5-novar-novar-phylo-', run_date, '.stan'),
                                    paste0('5-novar-novar-surv-phylo', run_date, '.R')))
...
j_idx2 <- dplyr::left_join(data.frame(species = tree_n$tip.label), 
                           data.frame(idx = 1:NROW(bird_df5), bird_df5), 
                           by = 'species')
#apply
bird_gls <- bird_df5[j_idx2$idx,]
library(nlme)
