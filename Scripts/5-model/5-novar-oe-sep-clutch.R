####################
# Fit Bayes model - clutch size ~ env
# Estimate clutch size from traits and phylo
...
#idx observed data
obs_idx <- which(bird_df3$SD_log_clutch_size == 0)
imp_idx <- which(bird_df3$SD_log_clutch_size != 0)

DATA <- list(No = length(obs_idx),
             Ni = length(imp_idx),
             y_obs = bird_df3$Phylo_log_clutch_size[obs_idx],
             y_imp = bird_df3$Phylo_log_clutch_size[imp_idx],
             sd_y = bird_df3$SD_log_clutch_size[imp_idx],
...
MCMCvis::MCMCdiag(fit, 
                  round = 4,
                  file_name = paste0('se-bird-novar-oe-sep-clutch-results-', run_date),
                  dir = paste0(dir, 'Results'),
                  mkdir = paste0('se-bird-novar-oe-sep-clutch-', run_date),
                  probs = c(0.055, 0.5, 0.945),
                  pg0 = TRUE,
                  save_obj = TRUE,
                  obj_name = paste0('se-bird-novar-oe-sep-clutch-fit-', run_date),
                  add_obj = list(DATA),
                  add_obj_names = paste0('se-bird-novar-oe-sep-clutch-data-', run_date),
                  cp_file = c(paste0(dir, 'Scripts/Model_files/5-novar-oe-sep.stan'), 
                              paste0(dir, 'Scripts/5-model/5-novar-oe-sep-clutch.R')),
                  cp_file_names = c(paste0('5-novar-novar-oe-sep-', run_date, '.stan'),
                                    paste0('5-novar-novar-oe-sep-clutch-', run_date, '.R')))
...
#lambda ~ 
# library(phytools)
# phytools::phylosig(tree_n, resid_srt, method = 'K') #quite slow
...
library(nlme)
pgls_fit <- nlme::gls(Phylo_log_clutch_size ~ lMass +
                        temp_sd_season +
                        temp_sd_year +
                        precip_cv_season +
                        precip_cv_year,
...
# (Intercept)       1.3627958 0.18884550  7.216459  0.0000
# lMass            -0.0441611 0.00455589 -9.693186  0.0000
# temp_sd_season    0.0233277 0.00150756 15.473805  0.0000
# temp_sd_year      0.0288011 0.02286626  1.259546  0.2079
# precip_cv_season  0.0291947 0.01316353  2.217846  0.0266
# precip_cv_year   -0.0029935 0.05794221 -0.051664  0.9588

#just measured
bird_ms <- dplyr::filter(bird_df3, !is.na(Measured_log_clutch_size))
...
library(nlme)
pgls_fit2 <- nlme::gls(Measured_log_clutch_size ~ lMass +
                         temp_sd_season +
                         temp_sd_year +
                         precip_cv_season +
                         precip_cv_year,
...
# Coefficients:
#   Value  Std.Error   t-value p-value
# (Intercept)       1.2972448 0.21898118  5.924002  0.0000
# lMass            -0.0580379 0.00907103 -6.398159  0.0000
# temp_sd_season    0.0532936 0.00360298 14.791535  0.0000
# temp_sd_year      0.0523754 0.05974645  0.876627  0.3808
# precip_cv_season  0.1369066 0.03290917  4.160134  0.0000
# precip_cv_year   -0.1311419 0.15124491 -0.867083  0.3860
...
fig_dir <- paste0(dir, 'Results/se-bird-novar-oe-sep-clutch-', run_date, '/')
