####################
# Fit Bayes model - surv ~ env + phylo + vint
####################
...
fit_ka <- geiger::fitContinuous(pr_tree2, dd[,'Modeled_survival'], model = "kappa")
...
lMass_scalar <- 0.25
temp_sd_season_scalar <- 0.06
temp_sd_year_scalar <- 0.2
precip_cv_season_scalar <- 0.15
precip_cv_year_scalar <- 0.25
y_scalar <- 4
...
DATA <- list(N = NROW(bird_df4),
             Y = bird_df4$Modeled_survival * y_scalar,
             K = NCOL(tt),
...
MCMCvis::MCMCdiag(fit, 
                  round = 4,
                  file_name = paste0('bird-surv-phylo-vint-results-', run_date),
                  dir = paste0(dir, 'Results'),
                  mkdir = paste0('bird-surv-phylo-vint-', run_date),
                  probs = c(0.055, 0.5, 0.945),
                  pg0 = TRUE,
                  save_obj = TRUE,
                  obj_name = paste0('bird-surv-phylo-vint-fit-', run_date),
                  add_obj = list(DATA),
                  add_obj_names = paste0('bird-surv-phylo-vint-data-', run_date),
                  cp_file = c(paste0(sc_dir, 'Scripts/Model_files/5-phylo-vint.stan'), 
                              paste0(sc_dir, 'Scripts/5-model/5-surv-phylo-vint.R')),
                  cp_file_names = c(paste0('5-phylo-vint-', run_date, '.stan'),
                                    paste0('5-surv-phylo-vint-', run_date, '.R')))

fig_dir <- paste0(dir, 'Results/bird-surv-phylo-vint-', run_date, '/')

# fit <- readRDS(paste0(dir, '/Results/bird-surv-phylo-vint-', run_date,
#                       '/bird-surv-phylo-vint-fit-', run_date, '.rds'))
# library(shinystan)
