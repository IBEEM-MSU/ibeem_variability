...
save(BL_usp, LH_usp, AV_usp, file = paste0(out_dir, 'range/bird-breeding/species-names-all-data.rda'))
...
BL.name.df <- data.frame(name = BL.sp.names, 
			 BL.id = 1:n_distinct(BL.sp.names))

# Complete DF
full.name.df <- full_join(BL.name.df, AV.sp.names, by = 'name')
full.name.df <- full_join(full.name.df, LH.sp.names, by = 'name')
write.csv(full.name.df, file = paste0(out_dir, 'trait/bird-names-full.csv'), row.names = FALSE)
...
LH_data <- left_join(LH_data, full.name.dat, by = c('Sci_name' = 'name'))
write.csv(LH_data, file = paste0(out_dir, 'trait/bird-et-al-data-with-id.csv'), 
	  row.names = FALSE)
AV_data <- AV_data %>%
  mutate(Species1 = tolower(Species1))
AV_data <- left_join(AV_data, full.name.dat, by = c('Species1' = 'name'))
write.csv(AV_data, file = paste0(out_dir, 'trait/avonet-with-id.csv'), 
	  row.names = FALSE)
BL_data <- BL_data %>%
  mutate(sci_name = tolower(sci_name))
BL_data <- left_join(BL_data, full.name.dat, by = c('sci_name' = 'name'))
...
for (i in 1:length(BL.unique.ids)) {
  ...
  tmp <- BL_data_breeding %>%
    filter(BirdLife_ID == BL.unique.ids[i])
