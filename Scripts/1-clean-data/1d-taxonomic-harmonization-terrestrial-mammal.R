...
# Life history data from Pacifici (LH)
LH_data <- read.csv(paste0(life_history_dir, 'Generation Length for Mammals.csv')) %>% 
  rename(name_pacifici = Scientific_name)

# Name data for phylacine (NM)
NM_data <- read.csv(paste0(life_history_dir, 'phylacine_data/Synonymy_table_valid_species_only.csv'))

# ID name inconsistencies between range and trait dbs and fix ------------------------------------

# This section only needs to be run once to generate mammal-names-matched.csv and mammal-names-no-match.csv
# These names were then manually reconciled and saved as mammal-names-fixed.csv 
# mammal-names-matched.csv and mammal-names-fixed.csv are later loaded into the script to 
...
# Save master file 
write.csv(master_names, "./data/L1/trait/mammal-names-master.csv", row.names = FALSE)
