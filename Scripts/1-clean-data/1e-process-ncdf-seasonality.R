################
# Convert netcdf ERA data into df, seasonality of specified months
#
# seasonality defined as sd of monthly means (as used by BioClim)
# args:
# - in dir
# - out dir
# - months to get seasonality over (separated by comma, no space)
################
...
#args[1] = in directory (that contains both temp and precip data) - '/mnt/research/ibeem/variability/data/L0/climate/era5/'
#args[2] = out directory - where to write file - '/mnt/research/ibeem/variability/data/L1/climate/era5/'
                     ...
                     months = 1)
{
  ...
  for (i in 1:length(files_temp)) #assuming num of temp files are the same as num precip
  {
    ...
    if (length(months) > 1)
    {
      var_array_temp3 <- apply(var_array_temp2, c(1, 2), sd)
      var_array_precip3 <- apply(var_array_precip2, c(1, 2), sd)
    } else {
    ...
    var_array_temp3_long <- as.vector(var_array_temp3)
    var_array_precip3_long <- as.vector(var_array_precip3)
    
    # create a dataframe
    llt <- expand.grid(lon, lat, lubridate::year(ymd_dates)[1])
    tmp_df <- data.frame(llt, var_array_temp3_long, var_array_precip3_long)
    
    colnames(tmp_df) <- c('lon', 'lat', 'year', 'sd_temp', 'sd_precip')
...
#take mean of sd across all years
env_out2 <- dplyr::group_by(env_out, lat, lon) %>%
  dplyr::summarize(mn_sd_temp = mean(sd_temp),
                   mn_sd_precip = mean(sd_precip)) %>%
  dplyr::ungroup()
...
#ERA5-MONTHS.csv
write.csv(env_out2, paste0(args[2], 'Env-seasonality-', 
